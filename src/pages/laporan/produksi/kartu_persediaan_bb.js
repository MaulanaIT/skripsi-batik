        ...
        axios.post(`${baseURL}/api/laporan/produksi/bahan-baku/select.php`, formData, config).then(response => {
            ...
            if (data && data.length > 0) {
                data.forEach(item => {
                    ...
                    master.harga_saldo = +master.unit_saldo === 0 ? 0 : Math.ceil(+master.jumlah_saldo / +master.unit_saldo);
                });
            }
            ...
            if (master) {
                htmlTableDaftarBahanBaku.push(
                    <tr key={'-1'}>
                        ...
                        <td>{SetPriceFormat(master.harga_saldo)}</td>
            ...
            if (data && data.length > 0) {
                data.forEach((item, index) => {
                    ...
                    currentHargaSaldo = +currentUnitSaldo === 0 ? 0 : Math.ceil(+currentJumlahSaldo / +currentUnitSaldo);
                ...
                <div className={`${global.card} col-12`}>
                    <div className='d-flex'>
                        ...
                        <div className={`${global.cursor_pointer} ms-auto pe-5`}>
                            <CSVLink data={getDataExport} filename={`Laporan Kartu Bahan Baku ${getValueTanggalAwal} - ${getValueTanggalAkhir}`}>
