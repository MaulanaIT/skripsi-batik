...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        (SELECT b.tanggal, 'Pembelian Alat' AS keterangan, a.jumlah AS unit_masuk, a.total_kapasitas AS kapasitas_masuk, 0 AS unit_keluar, 0 AS kapasitas_keluar, 0 AS unit_saldo, 0 AS kapasitas_saldo FROM detail_order_pembelian a INNER JOIN order_pembelian b ON a.kode = b.kode WHERE a.kode_item = '".$kode_item."' AND b.status = 3
        UNION ALL
        ...
        UNION ALL
        SELECT updated_at AS tanggal, 'Produksi' AS keterangan, 0 AS unit_masuk, 0 AS kapasitas_masuk, jumlah AS unit_keluar, jumlah AS kapasitas_keluar, 0 AS unit_saldo, 0 AS kapasitas_saldo from hpp_detail_alat WHERE kode_alat = '".$kode_item."') a";
