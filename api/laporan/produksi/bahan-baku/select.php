...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // } else if ($jenis_laporan == 'bahan baku') {
        ...
        (SELECT b.tanggal, 'Pembelian Bahan Baku' AS keterangan, a.jumlah AS unit_masuk, a.harga AS harga_masuk, 0 AS unit_keluar, 0 AS harga_keluar, 0 AS unit_saldo, 0 AS harga_saldo FROM detail_order_pembelian a INNER JOIN order_pembelian b ON a.kode = b.kode WHERE a.kode_item = '".$kode_item."' AND b.status = 3
        UNION ALL
        ...
        UNION ALL
        SELECT updated_at AS tanggal, 'Produksi' AS keterangan, 0 AS unit_masuk, 0 AS harga_masuk, jumlah AS unit_keluar, harga AS harga_keluar, 0 AS unit_saldo, 0 AS harga_saldo from hpp_detail_bahan_baku WHERE kode_bahan_baku = '".$kode_item."') a";
    // }
